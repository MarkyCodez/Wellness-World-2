  ...
  return (
    <Card className="rounded-[2rem] border-none shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Button variant="ghost" size="icon" onClick={onAddGoal} className="rounded-full">
      ...
      <CardContent className="space-y-6">
        {goals.length === 0 ? (
          <div className="text-center py-8 space-y-3">
            ...
            <Button variant="outline" onClick={onAddGoal} className="rounded-xl text-xs">Set Your First Goal</Button>
          </div>
        ) : (
          goals.map((goal) => {
